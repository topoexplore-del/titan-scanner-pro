"""
TITAN SCANNER PRO — Build Data Pipeline v6.0
...
def run_backtest_for_ticker(ticker, hist, ind, fund_data, is_etf=False):
    ...
    for i in range(start_bar, n, 5):
        ...
        if is_etf:
            ...
            l1 = (state in ('ENTRY+', 'ENTRY') and sc >= 60 and ai_v >= 65 
                  and abc in ('A', 'B') and rsi_v < 75)
        
        # Layer 2: Analysis (fundamentals — constant proxy)
        if is_etf:
            l2 = sc >= 30 and d20 > -5
        else:
            l2 = (composite >= 55 and eps_g > 0 and roe >= 8 and roa >= 3 
        ...
        for period in HOLD_PERIODS:
            ...
            # Nash: balanced (l1 + l2 + l3 but softer l4)
            nash_pass = l1 and l2 and l3 and bayesian_prob >= 0.55
            if nash_pass:
                gt_trades[period["key"]]["nash"].append(trade)
            
            # Bayesian: heavy AI weight (l2 + l4 + ai >= 60)
            bay_pass = l2 and l4 and ai_v >= 60 and d20 > -3
            if bay_pass:
                gt_trades[period["key"]]["bayesian"].append(trade)
            
            # Sequential: only strong signals (l1 + state active)
            seq_pass = l1 and l3 and state in ('ENTRY+', 'ENTRY', 'ACCUM')
            if seq_pass:
...
def main():
    ...
    print(f"TITAN SCANNER PRO — Building data at {datetime.now()}")
    groups_data = {}
